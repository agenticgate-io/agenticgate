...
#
# Generates a sanitized, deterministic demo transcript for AgenticGate.
...
yellow "▶ AgenticGate demo transcript generator"
...
if [[ "$CI_OFFLINE" == "true" ]]; then
  ...
  DEMO_MODE=true PORT_API=3001 NODE_ENV=test \
    npx --workspace @agenticgate/api ts-node src/index.ts &
  API_PID=$!
...
DEMO_MODE=true API_BASE_URL="$API_BASE" \
  npm run demo:transcript --workspace @agenticgate/agent-client
