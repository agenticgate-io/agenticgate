...
import type { PaymentAttempt, UsageEvent } from "@agenticgate/shared";
...
export function createTempAnalyticsDbPath(): string {
  return `/tmp/agenticgate-analytics-${randomUUID()}.db`;
}

export function createTempJsonPath(): string {
  return `/tmp/agenticgate-legacy-${randomUUID()}.json`;
}
